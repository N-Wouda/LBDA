...
void Gomory::update(double *rhs, int *vBasis, int *cBasis)
{
    ...
    {
        // lb = -inf if if var not at lower bound
        lb[var] = (vBasis[var] == -1) ? d_l2[var] : -inf;

        // ub =  inf if x[var] not at upper bound
        ub[var] = (vBasis[var] == -2) ? d_u2[var] : inf;
    }

    // setting the lower bounds to their appropriate values
    d_model.set(GRB_DoubleAttr_LB, d_vars, lb, d_n2);
    // setting the upper bounds to their appropriate values
    d_model.set(GRB_DoubleAttr_UB, d_vars, ub, d_n2);
}

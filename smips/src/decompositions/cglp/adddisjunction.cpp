    ...
    double *x, double theta, double *beta, double tau, double gamma)
{
    ...
    for (size_t var = 0; var != d_n1; ++var)
    {
        ...
        d_model.chgCoeff(d_constrs_x[2 * var][var],
                         d_lambda,
                         minus_beta[var] - tau);
        ...
        // \bar x_i - x_i
        // pi_theta
        d_model.chgCoeff(d_constrs_theta[2 * var + 1], d_lambda, 1.0);

        // pi_x
        d_model.chgCoeffs(d_constrs_x[2 * var + 1],
                          lambda_vector,
                          minus_beta,
                          d_n1);
        d_model.chgCoeff(d_constrs_x[2 * var + 1][var],
                         d_lambda,
                         minus_beta[var] + tau);

        // pi_0
        d_model.chgCoeff(d_constrs_cons[2 * var + 1],
                         d_lambda,
                         gamma + tau * x[var]);
    }
